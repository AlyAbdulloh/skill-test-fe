...
export const Residents: React.FC = () => {
  ...
  // Service configuration fallbacks
  const FILE_PATH = import.meta.env.VITE_FILE_PATH || 'http://localhost:8000/storage/';
  ...
  const fetchResidents = useCallback(async (page = 1) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        setErrorMessage(response.message || 'Gagal mengambil data penghuni.');
      }
    } catch (err: any) {
      console.error(err);
      setErrorMessage(
  ...
  // Reload residents on filter / search changes
  useEffect(() => {
    fetchResidents(1);
  ...
  const openEditModal = (resident: Resident) => {
    ...
    setImagePreview(resident.id_card_photo_url ? FILE_PATH + resident.id_card_photo : null);
    setFieldErrors({});
    setIsModalOpen(true);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.success) {
        showToast(res.message || 'Data penghuni berhasil dihapus.');
      ...
      console.error(err);
      setErrorMessage(
        err.response?.data?.message || 'Gagal menghapus data penghuni.'
      );
      setIsDeleteOpen(false);
    } finally {
      setLoading(false);
  ...
  return (
    <div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-base-content m-0">Manajemen Penghuni</h1>
          <p className="text-sm text-base-content/60">Lihat, tambah, dan kelola rincian data penghuni perumahan.</p>
        </div>
          ...
          <div className="overflow-x-auto w-full">
            {loading && residents.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 gap-3">
                ...
                <span className="text-sm text-base-content/60">Mengambil data penghuni...</span>
              </div>
            ) : (
                  ...
                  {residents.length > 0 ? (
                          ...
                          <div className="avatar">
                            <div className="w-16 h-10 rounded-lg border border-base-300 overflow-hidden bg-base-200">
                              {resident.id_card_photo_url ? (
                                ...
                                <div className="flex items-center justify-center h-full text-xs text-base-content/40 italic">Tidak ada foto</div>
                              )}
                            </div>
                          </div>
                        </td>
                    ...
                    <tr>
                      <td colSpan={6} className="text-center py-12 text-base-content/50">
                        Data penghuni tidak ditemukan. Tambahkan penghuni baru untuk memulai.
                      </td>
                    </tr>
                  )}
                </tbody>
        ...
        <div className="modal modal-open z-50 bg-black/60 transition-opacity duration-300">
          <div className="modal-box max-w-lg border border-base-200">
            <h3 className="font-bold text-lg text-base-content mb-4">
              {modalType === 'create' ? 'Tambah Profil Penghuni' : 'Ubah Profil Penghuni'}
            </h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="form-control">
                <label className="label py-1">
                  <span className="label-text font-semibold text-xs">Nomor Telepon (Hanya Angka)</span>
                </label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="form-control">
                  <label className="label py-1">
                    <span className="label-text font-semibold text-xs">Status Tinggal</span>
                  </label>
                  <select
                ...
                <div className="form-control">
                  <label className="label py-1">
                    <span className="label-text font-semibold text-xs">Status Pernikahan</span>
                  </label>
                  <select
                    value={isMarried ? '1' : '0'}
              ...
              <div className="form-control">
                <label className="label py-1">
                  <span className="label-text font-semibold text-xs">Foto KTP</span>
                </label>
                ...
                <div className="flex flex-col items-center justify-center border-2 border-dashed border-base-300 rounded-xl p-4 bg-base-200/20 hover:bg-base-200/50 transition-colors relative cursor-pointer group">
                  ...
                  <div className="text-center">
                    ...
                    <span className="text-xs text-base-content/65 block font-medium">Klik atau seret gambar ke sini untuk mengunggah</span>
                    <span className="text-[10px] text-base-content/40 block mt-0.5">JPG, PNG hingga 2MB</span>
                  </div>
                </div>
                ...
                {imagePreview && (
                  <div className="mt-3">
                    <p className="text-[11px] font-semibold text-base-content/50 mb-1">Pratinjau Gambar:</p>
      ...
      {isDeleteOpen && (
        <div className="modal modal-open z-50 bg-black/60">
          <div className="modal-box border border-base-200 text-center max-w-sm">
            ...
            <h3 className="font-bold text-lg text-base-content m-0">Konfirmasi Hapus</h3>
            <p className="text-sm text-base-content/60 mt-1 mb-6">
              Apakah Anda yakin ingin menghapus penghuni <strong>{deleteTarget?.full_name}</strong>? Tindakan ini tidak dapat dibatalkan.
            </p>
